...
namespace n902 {
template <typename R> struct step_iterator;

template <typename R> struct step_sentinel {
...
public:
  ...
  constexpr auto begin() {
    return step_iterator<R const>(std::ranges::begin(base_),
...
static_assert(std::sentinel_for<step_sentinel<test_range_t>,
...
namespace n903 {
template <typename R> struct replicate_iterator;

template <typename R> struct replicate_sentinel {
  using base = std::ranges::iterator_t<R>;
  using size_type = std::ranges::range_difference_t<R>;
...
template <typename R> struct replicate_iterator : std::ranges::iterator_t<R> {
  using base = std::ranges::iterator_t<R>;
  using value_type = typename std::ranges::range_value_t<R>;
  using reference_type = typename std::ranges::range_reference_t<R>;

  constexpr replicate_iterator(base start,
                               std::ranges::range_difference_t<R> count)
      : pos_{start}, count_{count} {}

  constexpr const replicate_iterator operator++(int) {
    if (step_ == count_) {
      step_ = 1;
      pos_++;
    } else {
  ...
  [[nodiscard]] constexpr base const value() const { return pos_; }
...
public:
  ...
  [[nodiscard]] constexpr R base() const &
    requires std::copy_constructible<R>
  {
    return base_;
  }
...
static_assert(std::sentinel_for<replicate_sentinel<test_range_t>,
                                replicate_iterator<test_range_t>>);
...
int main() {
...

  {
    ...
    auto even = std::count_if(a.begin(), a.end(),
...

  {
    std::array<int, 5> a{1, 5, 3, 2, 4};
    auto even =
        std::ranges::count_if(a, [](int const n) { return n % 2 == 0; });
    assert(even == 2);
  }
...

  {
    ...
    std::ranges::sort(v);
    auto r =
        v | std::ranges::views::filter([](int const n) { return n % 2 == 0; }) |
...
;
...

  {
    ...
    std::vector<Item> items{
        {1, "pen", 5.49}, {2, "ruler", 3.99}, {3, "pensil case", 12.50}};

    std::vector<Item> copies;

    std::ranges::copy_if(items, std::back_inserter(copies),
                         [](Item const &i) { return i.name[0] == 'p'; });

    copies.clear();

    std::ranges::copy_if(
        items, std::back_inserter(copies),
        [](std::string const &name) { return name[0] == 'p'; }, &Item::name);

    copies.clear();

    std::vector<std::string> names;
    std::ranges::copy_if(
        items | rv::transform(&Item::name), std::back_inserter(names),
