...
struct SizeIterator {
  ...
  operator!=(const SizeIterator &other) const noexcept {
    return loc != other.loc;
  }

  [[nodiscard]] constexpr const std::pair<std::size_t, std::size_t> &
...
constexpr auto opt_pow(const std::complex<ComplexType> &t_val,
                       PowerType t_power) {
  if (t_power == static_cast<PowerType>(1.0)) {
    return t_val;
  } else if (t_power == static_cast<PowerType>(2.0)) {
    return std::complex{pow<2>(std::real(t_val)) - pow<2>(std::imag(t_val)),
                        2 * std::real(t_val) * std::imag(t_val)};
  } else if (t_power == static_cast<decltype(t_power)>(3.0)) {
    const auto a = std::real(t_val);
    const auto b = std::imag(t_val);
    return std::complex{-3 * a * pow<2>(b) + pow<3>(a),
                        3 * pow<2>(a) * b - pow<3>(b)};
...
constexpr auto get_color(const Point<PointType> t_point,
                         const Point<CenterType> t_center, const Size t_size,
                         const ScaleType t_scale, std::size_t max_iteration,
                         const CenterType power, const bool do_abs) noexcept {
  const std::complex scaled{
      t_point.x / (t_size.width / t_scale) +
          (t_center.x - (t_scale / static_cast<CenterType>(2.0))),
      t_point.y / (t_size.height / t_scale) +
          (t_center.y - (t_scale / static_cast<CenterType>(2.0)))};
  ...
  while (iteration < stop_iteration) {
    ...
    if (do_abs) {
      current = std::complex{std::abs(std::real(current)),
                             std::abs(std::imag(current))};
    }
  ...
  if (iteration == max_iteration) {
    ...
    const auto value =
        ((iteration + 1) - (std::log(std::log(std::abs(std::real(current) *
                                                       std::imag(current))))) /
                               std::log(power));
    ...
    default:
      return Color{.988, .027, .910};
    }
  }
}

template <typename PointType, typename ColorType>
void set_pixel(sf::Image &img, const Point<PointType> &t_point,
               const Color<ColorType> &t_color) {
...
template <std::size_t Width, std::size_t Height> struct Image {
  ...
  const auto &operator[](const std::pair<std::size_t, std::size_t> &loc) const {
    return colors[loc.second * Width + loc.first];
  }
  auto &operator[](const std::pair<std::size_t, std::size_t> &loc) {
    return colors[loc.second * Width + loc.first];
  }
...
template <std::size_t Width, std::size_t Height> constexpr auto get_indicies() {
  ...
  std::generate(
      begin(indicies), end(indicies),
      [index = begin(Size{Width, Height})]() mutable { return *(index++); });
  return indicies;
}

template <std::size_t Width, std::size_t Height>
void run(Image<Width, Height> *img, const Settings *global_settings) {
  auto localImg = std::make_unique<Image<Width, Height>>(*img);
  auto settings = *global_settings;
  static constexpr const auto indicies = get_indicies<Width, Height>();
  ...
  while (!settings.canceling) {
    ...
    if (cur_max_iterations <= max_max_iterations) {
      ...
      std::transform(std::execution::par_unseq, begin(indicies), end(indicies),
                     begin(localImg->colors), [=](const auto &location) {
                       return get_color(Point{location.first, location.second},
                                        settings.center, size, settings.scale,
                                        cur_max_iterations, settings.power,
                                        settings.do_abs);
                     });
...
int main() {
  ...
  while (window.isOpen()) {
    ...
    for (const auto &loc : size) {
      set_pixel(img, Point{loc.first, loc.second}, (*img_copy)[loc]);
    }
    ...
    settings = [settings = Settings(settings)]() mutable {
      ...
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
        settings.center.x -= move_offset;
      }
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) {
        settings.center.x += move_offset;
      }
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
        settings.center.y -= move_offset;
      }
      ...
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
        settings.do_abs = !settings.do_abs;
      }
