...
struct knight_mage : game_unit {
  void attack() override { std::cout << "draw magic sword\n"; }
};
...
attack operator+(attack const &a, int value) { return attack{a.value + value}; }

knight_mage operator+(knight const &k, mage const &m) { return knight_mage{}; }
...
template <typename T> void process(Base<T> &b) { b.f(); }
} // namespace n702

namespace n703 {
template <typename T> struct game_unit {
...
template <typename T> void fight(std::vector<game_unit<T> *> const &units) {
  for (auto unit : units) {
    unit->attack();
  }
}
...
namespace n704 {
template <typename T, size_t N> struct limited_instances {
...
namespace n705 {
template <typename T> struct movable_unit {
...
template <typename T> void advance(T &t, size_t steps) {
  while (steps--)
    t.step_forth();
}

template <typename T> void retreat(T &t, size_t steps) {
  while (steps--)
    t.step_back();
}
} // namespace n706
...
namespace n708 {
template <typename T> struct base_unit {
  template <typename U> void ally_with(U &other);
};
...
private:
  ...
  template <typename U> friend struct base_unit;
...
namespace n710a {
struct hit_and_run {
  void fight() { std::cout << "hit once hard then run\n"; }
};

struct last_man_standing {
  void fight() { std::cout << "duel until one falls\n"; }
};

template <typename Strategy> struct knight : public Strategy {
  void attack() {
    std::cout << "draw sword\n";
    Strategy::fight();
  }
...
template <typename Strategy> struct mage : public Strategy {
  void attack() {
...
template <typename T> struct movable_unit : T {
  void advance(size_t steps) {
    while (steps--)
      T::step_forth();
  }
...
template <typename T> struct lone_warrior : T {
  void fight() {
...
template <typename T> struct team_warrior : T {
  void fight() {
...
template <typename T> struct knight : T, game_unit {
  void attack() {
    std::cout << "draw sword.";
    T::fight();
  }
...
template <typename T> struct mage : T, game_unit {
  void attack() {
    std::cout << "spell magic curse.";
    T::fight();
  }
...
template <typename Iter, typename Distance> void advance(Iter &it, Distance n) {
...
namespace n711b {
template <std::random_access_iterator_tag Iter, typename Distance>
...
template <typename T> struct game_unit_wrapper : public game_unit {
...
struct game {
  ...
  template <typename T> void addUnit(T &unit) {
...
struct unit {
  ...
  template <typename T> struct unit_model : public unit_concept {
...
void fight_knight(void *k) { reinterpret_cast<knight *>(k)->attack(); }

void fight_mage(void *m) { reinterpret_cast<mage *>(m)->attack(); }
...
template <typename TL> struct front_type;
...
template <template <typename...> typename TL> struct front_type<TL<>> {
  using type = empty_type;
};

template <typename TL> struct back_type;
...
template <template <typename...> typename TL> struct back_type<TL<>> {
  using type = empty_type;
};

template <typename TL, typename T> struct push_back_type;
...
template <typename TL, typename T> struct push_front_type;
...
template <typename TL> struct pop_front_type;
...
template <template <typename...> typename TL> struct pop_front_type<TL<>> {
  using type = TL<>;
};

template <std::ptrdiff_t N, typename R, typename TL> struct pop_back_type;
...
template <> struct pop_back_type<-1, typelist<>, typelist<>> {
  using type = typelist<>;
};

template <std::size_t I, std::size_t N, typename TL> struct at_type;

template <std::size_t I, std::size_t N, template <typename...> typename TL,
          typename T, typename... Ts>
struct at_type<I, N, TL<T, Ts...>> {
  using type = std::conditional_t<I == N, T,
                                  typename at_type<I, N + 1, TL<Ts...>>::type>;
};

template <std::size_t I, std::size_t N> struct at_type<I, N, typelist<>> {
  using type = empty_type;
};
} // namespace detail

template <typename TL> using length_t = typename detail::length<TL>::type;

template <typename TL> constexpr std::size_t length_v = length_t<TL>::value;
...
template <typename TL> using front_t = typename detail::front_type<TL>::type;
...
template <typename TL> using back_t = typename detail::back_type<TL>::type;
...
static_assert(
    std::is_same_v<push_back_t<typelist<char>, int>, typelist<char, int>>);
static_assert(std::is_same_v<push_back_t<typelist<double, char>, int>,
...
static_assert(
    std::is_same_v<push_front_t<typelist<char>, int>, typelist<int, char>>);
static_assert(std::is_same_v<push_front_t<typelist<double, char>, int>,
...
static_assert(
    std::is_same_v<pop_front_t<typelist<double, char>>, typelist<char>>);

template <typename TL>
using pop_back_t = typename detail::pop_back_type<
    static_cast<std::ptrdiff_t>(length_v<TL>) - 1, typelist<>, TL>::type;

static_assert(std::is_same_v<pop_back_t<typelist<>>, typelist<>>);
static_assert(std::is_same_v<pop_back_t<typelist<double>>, typelist<>>);
static_assert(
    std::is_same_v<pop_back_t<typelist<double, char>>, typelist<double>>);
static_assert(std::is_same_v<pop_back_t<typelist<double, char, int>>,
...
struct upgrade_attack {
  void operator()(game_unit &u) { u.attack += 2; }
};

using namespace n714;

template <typename TL> struct apply_functors {
private:
  template <size_t I> static void apply(game_unit &unit) {
    using F = at_t<I, TL>;
    std::invoke(F{}, unit);
  }
...
template <typename... Ts> struct transformer {
  using input_types = typelist<Ts...>;
  using output_types = typelist<std::add_const_t<Ts>...>;
};

static_assert(std::is_same_v<transformer<int, double>::output_types,
...
template <typename T> struct vector {
  ...
  T const &operator[](const std::size_t i) const { return data_[i]; }
...
namespace n717 {
template <typename T, typename C = std::vector<T>> struct vector {
  vector() = default;
...
template <typename L, typename R> struct vector_add {
...
template <typename L, typename R> struct vector_mul {
...
template <typename S, typename R> struct vector_scalar_mul {
