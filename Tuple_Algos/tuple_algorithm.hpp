...
constexpr decltype(auto) count(T &&t, const U &value) {
  return count_if([&]<typename Args>(Args &&args) { return args == value; },
                  std::forward<T>(t));
}
...
constexpr std::optional<std::size_t> mismatch(F &&f, T &&t, U &&u) {
  ...
  if constexpr (N == 0)
    return std::nullopt;
                        ...
                        std::get<i>(std::forward<T>(u))))
    return std::make_optional(i);
  else
    return mismatch<N - 1>(std::forward<F>(f), std::forward<T>(t),
                           std::forward<U>(u));
}

template <typename T, typename U>
constexpr decltype(auto) mismatch(T &&t, U &&u) {
  return mismatch<tuple_size_v<T>>([](auto &&l, auto &&r) { return l == r; },
                                   std::forward<T>(t), std::forward<U>(u));
}

template <typename F, typename T, typename U>
constexpr decltype(auto) mismatch(F &&f, T &&t, U &&u) {
  return mismatch<tuple_size_v<T>>(std::forward<F>(f), std::forward<T>(t),
                                   std::forward<U>(u));
}
