...
static_assert(std::is_same_v<pop_front_t<type_list<int, bool, float>>,
...
static_assert(std::is_same_v<push_back_t<type_list<int, bool>, float>,
...
template <template <typename...> class LIST, typename... ELEMS,
          template <typename...> class TO_LIST, typename... ELEMS2>
struct make_same_container<LIST<ELEMS...>, TO_LIST<ELEMS2...>>
    : has_type<TO_LIST<ELEMS...>> {};

template <typename FROM_LIST, typename TO_LIST>
using make_same_container_t =
    typename make_same_container<FROM_LIST, TO_LIST>::type;

template <typename LIST,
          typename RET_LIST = make_same_container_t<type_list<>, LIST>>
struct pop_back;
...
template <template <typename...> class LIST, typename T0, typename T1,
          typename... T2toN, typename RET_LIST>
...
static_assert(std::is_same_v<pop_back_t<type_list<int, bool, float>>,
                             type_list<int, bool>>);
static_assert(std::is_same_v<pop_back_t<type_list<int, bool>>, type_list<int>>);
static_assert(
    std::is_same_v<pop_back_t<std::tuple<int, bool>>, std::tuple<int>>);
