...
namespace ia {
template <typename T, typename Tuple> struct is_among;
...
static_assert(ia::is_among_v<int, std::tuple<char, int, double>>);
static_assert(!ia::is_among_v<float, std::tuple<char, int, double>>);
