...
namespace n406 {
template <typename T> struct base_parser {
  template <typename U> void init() { std::cout << "init\n"; }
...
namespace n409b {
template <unsigned int n> constexpr unsigned int factorial() {
...
template <typename T> void process01(T) { std::cout << "T" << '\n'; }

template <typename T> void process02(T const) {
  std::cout << "T const" << '\n';
}

template <typename T> void process03(T volatile) {
  std::cout << "T volatile" << '\n';
}
...
template <size_t n> void process07(account_t[5][n]) {
  std::cout << "C[5][n]" << '\n';
}

template <typename T> void process08(T (*)()) {
  std::cout << "T (*)()" << '\n';
}

template <typename T> void process08(account_t (*)(T)) {
  std::cout << "C (*)(T)" << '\n';
}

template <typename T, typename U> void process08(T (*)(U)) {
  std::cout << "T (*)(U)" << '\n';
}

template <typename T> void process09(T (account_t::*)()) {
  std::cout << "T (C::*)()" << '\n';
}

template <typename T, typename U> void process09(T (account_t::*)(U)) {
  std::cout << "T (C::*)(U)" << '\n';
}

template <typename T, typename U> void process09(T (U::*)()) {
  std::cout << "T (U::*)()" << '\n';
}

template <typename T, typename U, typename V> void process09(T (U::*)(V)) {
  std::cout << "T (U::*)(V)" << '\n';
}

template <typename T> void process09(account_t (T::*)()) {
  std::cout << "C (T::*)()" << '\n';
}

template <typename T, typename U> void process09(transaction_t (T::*)(U)) {
  std::cout << "C (T::*)(U)" << '\n';
}

template <typename T> void process09(balance_report_t (balance_t::*)(T)) {
  std::cout << "D (C::*)(T)" << '\n';
}

template <typename T> void process10(T account_t::*) {
  std::cout << "T C::*" << '\n';
}

template <typename T> void process10(account_t T::*) {
  std::cout << "C T::*" << '\n';
}

template <typename T, typename U> void process10(T U::*) {
  std::cout << "T U::*" << '\n';
}
...
template <typename T> void process11(wrapper<T>) {
  std::cout << "C<T>" << '\n';
}

template <size_t i> void process12(int_array<i>) {
  std::cout << "C<i>" << '\n';
}

template <template <typename> class TT, typename T> void process13(TT<T>) {
  std::cout << "TT<T>" << '\n';
}

template <template <size_t> typename TT, size_t i> void process14(TT<i>) {
  std::cout << "TT<i>" << '\n';
}

template <template <typename> typename TT> void process15(TT<account_t>) {
  std::cout << "TT<C>" << '\n';
}
} // namespace n412

namespace n413 {
template <typename T, T i> void process(double arr[i]) {
  using index_type = T;
...
namespace n414 {
template <typename T> void process(T a = 0, T b = 42) {
  std::cout << a << "," << b << '\n';
}
} // namespace n414

namespace n415 {
template <typename T> void invoke(void (*pfun)(T, int)) { pfun(T{}, 42); }

template <typename T> void alpha(T, int) {
...
namespace n416 {
template <size_t Size> void process1(int a[Size]) {
  std::cout << "process(int[Size])" << '\n';
}

template <size_t Size> void process2(int a[5][Size]) {
  std::cout << "process(int[5][Size])" << '\n';
}

template <size_t Size> void process3(int (&a)[Size]) {
  std::cout << "process(int[Size]&)" << '\n';
}

template <size_t Size> void process4(int (*a)[Size]) {
  std::cout << "process(int[Size]*)" << '\n';
}
} // namespace n416
...
template <size_t N> void process(ncube<N - 1> cube) {
...
template <typename T> constexpr wrapper<T> make_wrapper(T &&data) {
  return wrapper{data};
}
...
auto make_vector(T &&first, Ts &&...args) {
  return std::vector<std::decay_t<T>, Allocator>{std::forward<T>(first),
                                                 std::forward<Ts>(args)...};
}
} // namespace n418
...
template <typename T> void h(T &&v) { g(std::forward<T>(v)); }
} // namespace n426
...
namespace n432 {
template <typename T, typename U> decltype(auto) minimum(T &&a, U &&b) {
  return a < b ? a : b;
}
} // namespace n432

namespace n433 {
template <typename T, typename U> auto minimum(T &&a, U &&b) {
  return a < b ? a : b;
}
} // namespace n433

namespace n434 {
template <typename T> T const &func(T const &ref) { return ref; }

template <typename T> auto func_caller(T &&ref) {
...
template <typename T> decltype(auto) func_caller(T &&ref) {
  return func(std::forward<T>(ref));
}
} // namespace n435
...
void print(wrapper const &w) { std::cout << w.value << '\n'; }
} // namespace n438
...
template <> void print<int>(wrapper const &w) { std::cout << w.value << '\n'; }
...
void print(wrapper<int> const &w) { std::cout << w.value << '\n'; }
...
template <typename T> void print(wrapper<T> const &w) {
  // std::cout << w.value << '\n'; // error
}

template <> void print(wrapper<int> const &w) { std::cout << w.value << '\n'; }
...
template <> struct printer<int> {
  void operator()(wrapper<int> const &w) { std::cout << w.value << '\n'; }
};
} // namespace n442
...
template <typename T> struct printer {
  void operator()(wrapper<T> const &w) { std::cout << w.value << '\n'; }
};
} // namespace n443
...
template <typename T> struct dictionary : T::map_type {
  ...
  using value_type = typename T::map_type::mapped_type;

  void add(typename T::key_type const &, value_type const &) {}
};
} // namespace n446
...
int main() {
...

  {
    ...
    f(x);  // f(foo&)
    f(rx); // f(foo&)
    // f(foo{ 42 });     // error, a non-const reference may only be bound to an
    // lvalue
  }
...

  {
    ...
    static_assert(
        std::is_same_v<double, composition<int, double>::result_type>);
...

  {
    ...
    static_assert(
        std::is_same_v<wrapper, composition<int, wrapper>::result_type>);
  }
