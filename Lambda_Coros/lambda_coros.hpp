...
namespace cppshizoid::tools::lambda_coros {
...
constexpr auto lambda_co_range(Lambda lambda, std::size_t skip = 0,
    ...
    struct Range {
        ...
        struct Iterator {
            ...
            std::optional<std::size_t> length;
            std::reference_wrapper<std::size_t> stride_ref; // Add a reference to stride
            bool is_end = false;
...
template <typename Lambda>
constexpr auto while_has_value(Lambda lambda) {
    struct Range {
        Lambda generator;
