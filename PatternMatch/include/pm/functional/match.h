...
/**
 * @brief Функция-заглушка для динамического приведения типов.
 *
 * Функция-заглушка, которая предназначена для динамического приведения типов, но не реализована.
 */
template <typename>
void dyn_cast();
...
auto match(Type&& x) {
  return [x = std::forward<Type>(x)](auto&&... lambdas) mutable -> decltype(auto) {
    return ::pm::matcher(std::forward<decltype(lambdas)>(lambdas)...)(x);
  };
}
