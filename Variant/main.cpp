...
template <typename Head, typename... Tail> struct MaxSizeHelper {
  static constexpr size_t MaxSize() {
    return std::max(sizeof(Head), MaxSizeHelper<Tail...>::MaxSize());
  }
};

template <typename Head> struct MaxSizeHelper<Head> {
  static constexpr size_t MaxSize() { return sizeof(Head); }
};

template <typename... Types> constexpr size_t MaxSize() {
  return MaxSizeHelper<Types...>::MaxSize();
}

template <typename Head, typename... Tail> struct MaxAlignmentHelper {
  static constexpr size_t MaxAlignment() {
    return std::max(alignof(Head), MaxAlignmentHelper<Tail...>::MaxAlignment());
  }
};

template <typename Head> struct MaxAlignmentHelper<Head> {
  static constexpr size_t MaxAlignment() { return alignof(Head); }
};

template <typename... Types> constexpr size_t MaxAlignment() {
...
struct FindHelper<Needle, Needle, HayStackTail...> {
  static constexpr size_t Find() { return 0; }
};

template <typename Needle, typename... HayStack> constexpr size_t Find() {
  return FindHelper<Needle, HayStack...>::Find();
}

template <size_t I, typename... Types> struct ByIndex {};

template <size_t I, typename TypesHead, typename... TypesTail>
...
template <typename T> constexpr void Destroy(void *ptr) {
...
constexpr void Destroy(size_t typeIndex, void *ptr) {
...
public:
  ...
  template <typename Type> constexpr Variant &operator=(Type &&value) {
    this->~Variant();
