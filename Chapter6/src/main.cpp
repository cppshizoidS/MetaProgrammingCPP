...
template <arithmetic T> T add(T const a, T const b) { return a + b; }
} // namespace n606
...
template <container C> void process(C &&c) {}
} // namespace n611
...
template <logger T> void log_error(T &logger) {}
...
template <typename T, typename V> struct key_value_pair {
  using key_type = T;
  using value_type = V;
...
namespace n614 {
template <typename T> void f(T) noexcept {}
...
template <SignedIntegral T> T decrement(T value) { return value--; }
} // namespace n618
...
namespace n634 {
template <std::floating_point T> constexpr T PI = T(3.1415926535897932385L);
}

namespace n635a {
template <std::integral T> using integral_vector = std::vector<T>;
}
...
template <> auto add(char const *a, char const *b) {
...
namespace n636c {
auto add(std::integral auto... args) { return (args + ...); }
} // namespace n636c
