...
struct Token {
  ...
  constexpr auto is(TokenType other) const -> bool { return type == other; }

  constexpr auto is_end() const { return type == TokenType::End; }
};
...
struct TokenStream {
  ...
  constexpr auto token() const -> Token { return tokens[pos]; }

  constexpr auto at(size_t where) const -> TokenStream {
    return {tokens, where};
  }

  constexpr auto current() const -> size_t { return pos; }
