...
namespace detail {
template <typename T> consteval auto __compile(auto stream) {
...
template <typename T> consteval auto __list(auto stream, auto &&...args) {
  auto r = Wrapper<[] { return __compile<T>(decltype(stream){}); }>{};
  if constexpr (r.value()) {
    return __list<T>(Wrapper<[] {
                       return decltype(stream){}.value().at(
                           decltype(r){}.value().current);
                     }>{},
                     std::forward<decltype(args)>(args)..., r.value().value);
  } else {
    return Result{std::tuple{std::forward<decltype(args)>(args)...},
...
consteval auto __group(auto stream, auto &&...args) {
  ...
  if constexpr (r.value()) {
    if constexpr (sizeof...(Tail) > 0) {
      ...
      return Result{
          std::tuple{std::forward<decltype(args)>(args)..., r.value().value},
          r.value().current};
    }
  } else {
    return None{};
...
consteval auto __first(auto stream) { return None{}; }

template <typename Head, typename... Tail> consteval auto __first(auto stream) {
...
template <size_t id> consteval auto __get(const auto &leaf);
...
template <TokenType type> struct token {
  ...
  constexpr auto is(TokenType other) const { return type == other; }
};

template <size_t id> struct ident {
  consteval static auto compile(auto stream) {
    if constexpr (stream.value().token().is(TokenType::Identifier) &&
                  stream.value().token().id == id) {
      return Result{stream.value().token(), stream.value().current() + 1};
    } else {
      return None{};
    }
...
template <typename T> struct list_non_empty {
  consteval static auto compile(auto stream) {
...
template <typename... T> struct one_of {
  consteval static auto compile(auto stream) {
    return detail::__first<T...>(stream);
  }
};

template <size_t id, typename T> struct with_name : T {
...
template <const_string name> consteval auto get(const auto &tree) {
